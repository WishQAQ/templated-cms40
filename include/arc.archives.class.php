...
class Archives
{
    ...
    function GetPagebreakDM($totalPage, $nowPage, $aid)
    {
        ...
        if($nowPage==1)
        {
            $PageList.="<a href='#'>上页</a>";
        }
        else
        {
            if($nPage==1)
            {
                $PageList.="<a href='view.php?aid=$aid'>上页</a>";
                if($cfg_rewrite == 'Y')
                {
            ...
            {
                $PageList.="<a href='view.php?aid=$aid&pageno=$nPage'>上页</a>";
                if($cfg_rewrite == 'Y')
                {
        ...
        if($lPage <= $totalPage)
        {
            $PageList.="<a href='view.php?aid=$aid&pageno=$lPage'>下页</a>";
            if($cfg_rewrite == 'Y')
            {
                $PageList = str_replace(".php?aid=", "-", $PageList);
                $PageList =  preg_replace("#&pageno=(\d+)#i", '-\\1.html', $PageList);
    ...
    function GetPagebreak($totalPage, $nowPage, $aid)
    {
        ...
        else
        {
            if($nPage==1)
            {
                $PageList.="<a href='".$this->NameFirst.".".$this->ShortName."'>上页</a>";
            }
            else
            {
                $PageList.="<a href='".$this->NameFirst."_".$nPage.".".$this->ShortName."'>上页</a>";
            }
        }
        for($i=1;$i<=$totalPage;$i++)
        {
        ...
        if($lPage <= $totalPage)
        {
            $PageList.="<a href='".$this->NameFirst."_".$lPage.".".$this->ShortName."'>下页</a>";
        }
        else
        {
            $PageList.= "<a href='#'>下页</a>";
